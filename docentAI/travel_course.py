...
여행지와 그에 대한 설명은 다음과 같아. 각각 여행지에 대한 설명에 추가로, 멋진 여행스토리를 만들어줘. 스토리는 하나의 이야기처럼 잘 만들어줘. 이때 하루에 최소 2개 이상의 여행지로 코스를 만들어줘. 다른 인사말 같은거 하지말고, 바로 스토리에 대해서 이야기해줘. 그리고 강조할 문장이나 단어에는 볼드체 할 수 있도록 정규식으로 표시해줘. 뻔한 이야기는 하지말고, 각 관광지가 갖고있는 전설과 역사와 같이 알면 더 흥미롭게 여행할 수 있는 내용들을 포함해서 스토리를 만들어줘. 역사와 전설에 대한 이야기를 통해 여행자에게 감동을 주는 스토리로 만들어줘. 여행자가 여행지에 도착했을 때, 그곳의 분위기와 전설을 떠올리며 여행을 즐길 수 있도록 스토리를 만들어줘. 
그리고 마지막엔 어떤 관광지인지 알 수 있도록 관광지의 ID를 리스트 형태로 출력해줘. 이때 꼭, "여행스토리에 포함된 모든 관광지"의 ID를 다 포함해야해. 하나도 빠짐없이 모든 관광지의 ID를 리스트 형태로 출력해줘. 결과물은 항상 아래 예시로 들어준 json의 형태로 출력해줘.

expected_output:
```json
{{
    "recommendation": "아침 햇살이 제주도를 따뜻하게 감싸고 있어요. 오늘은 곽지해수욕장으로 가볼까요? 이곳에는요, 옛날 옛적에 **모래에 묻힌 마을**에 대한 전설이 있어요. 바람에 휩쓸려 그 마을이 모래 속으로 사라졌다는 이야기인데요, 지금도 이 해변을 찾는 사람들에게 뭔가 신비로운 상상을 불러일으키죠. 마치 모래 아래에 그 마을이 잠들어 있는 듯한 기분이 들 거예요. 해변을 걸으면 발밑으로 물결이 살짝 스치는데, 그 순간 모든 걱정이 사라지는 것처럼 편안해질 거예요. 해변 끝쪽에 **과물노천탕**이 있는데요, 바다에서 놀고 나서 따뜻한 물에 몸을 씻으며 여유로운 시간을 보내보세요. 그리고 이제 **애월 바다의 아름다움**을 즐길 수 있는 **제주올레 16코스**로 가볼까요? **고내에서 구엄까지** 이어지는 이 길은 **쪽빛 바다**와 **하얀 소금기**, 그리고 **나무 그늘**이 정말 아름답게 어우러져 있어요. 걷다 보면 정말 평화롭고, 마음이 차분해질 거예요. 점심시간에는 **애월한담공원**으로 가서 바다를 바라보며 산책을 즐기는 것도 좋아요. 여기서는 **물허벅 여인상**을 볼 수 있는데요, 제주의 여성들은 예로부터 물이 귀한 섬에서 **물허벅**을 머리에 이고 우물과 집을 오가며 가족의 생명을 지켰어요. 그 과정에서 서로 힘든 짐을 덜어주고 격려하며 강한 유대감을 쌓았다고 해요. 전설에 따르면, 어느 날 한 여인이 무거운 물허벅을 이고 걸어가는데 **하늘에서 신비로운 존재**가 나타나 물허벅을 가볍게 만들어주었다고 해요. 이곳에서 그 강인한 제주 여인들을 떠올리며 그들의 **의지**와 **자연과의 조화**를 느껴보세요. 점심으로는 **고기국수**가 유명한 **꽁순이네**를 추천해요. **진한 국물**이 제주에서만 느낄 수 있는 특별함을 더해줄 거예요. 식사 후에는 **애월 해안**의 **한담해안산책로**를 걸어보세요. **파도 소리**가 속삭이듯 발걸음을 맞이해줄 거예요. 이곳에서 오래전 **한 어부**가 바다의 신과 대화를 나누었다는 이야기도 있답니다. **거친 파도**에 시달리던 마을을 위해 어부가 진심을 담아 바다와 대화를 시도했는데, 그 진심이 전해져서 바다가 잔잔해졌다는 이야기가 전해져 내려오고 있죠. 그 이야기를 떠올리며 산책로를 따라 걸으면 마음도 차분해지고 바다의 평온함이 온몸에 스며드는 것 같은 기분이 들 거예요. 오후에는 **귀덕바다투명카약 체험**을 해보는 것도 좋아요. **투명한 카약** 아래로 보이는 다채로운 수중 생물들이 하늘과 바다의 경계를 허물며 정말 재미있는 경험을 선사할 거예요. 그 후 다시 **한담해변**으로 돌아가서 **일몰**을 감상해보세요. **붉게 물드는 하늘**을 바라보며 제주에서의 특별한 순간을 마음에 새길 수 있을 거예요. 저녁에는 **선운정사**로 가서 **노을빛에 비친 연꽃등**을 감상해보세요. **차분한 경전 소리**와 함께하는 이 불빛의 향연은 하루의 피로를 잊게 해줄 거예요. 이렇게 제주에서의 하루를 마무리하며, 마음이 한결 가벼워질 거예요. 이제 제주에서의 추억을 안고 돌아가면, 오늘 하루가 오랫동안 **그리움**으로 남을 거예요.",
    "ID": [123203, 126444, 126444, 126444, 126444]
}}
```
...
async def recommend_travel(request: RecommendTravelRequest):
    try:
        ...
        result = recommend_travel_spots(age, gender, request.season, request.duration, request.travelmate, filtered_df, "gpt-4o", CLIENT)
        print(result)
